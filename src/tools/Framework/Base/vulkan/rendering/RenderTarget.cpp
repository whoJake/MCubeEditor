...
{
    if( m_images.size() == 0 )
        throw VulkanException("Render target requires at least one image.");
    ...
    for( auto& image : m_images )
    {
        if( image.get_type() != VK_IMAGE_TYPE_2D )
            throw VulkanException("Images of a RenderTarget must be of type 2D.");

        VkExtent2D extent = image_to_extent_2D(image);
        if( hasher(extent) != uniqueHash )
            throw VulkanException("Cannot have more than one extent size in a Render Target.");
    ...
    m_imageViews(std::move(imageViews))
{
    if( m_imageViews.size() == 0 )
        throw VulkanException("Render target requires at least one image view.");
