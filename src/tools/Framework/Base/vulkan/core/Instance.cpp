...
static jclog::Log* s_vkDebugLog = nullptr;
...
static VKAPI_ATTR VkBool32 VKAPI_CALL debug_utils_callback(VkDebugUtilsMessageSeverityFlagBitsEXT severity, VkDebugUtilsMessageTypeFlagsEXT type, const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData, void* pUserData)
{
    if( severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT )
        JCLOG_ERROR(*s_vkDebugLog, pCallbackData->pMessage);
    else if( severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT )
        JCLOG_WARN(*s_vkDebugLog, pCallbackData->pMessage);
    else
        JCLOG_INFO(*s_vkDebugLog, pCallbackData->pMessage);
...
Instance::Instance(jclog::Log&                    log,
    ...
    m_enabledExtensions(enabledExtensions)
{
    ...
    for( const char* layerName : enabledValidationLayers )
    {
        ...
        if( !foundLayer )
        {
            JCLOG_ERROR(log, "Validation layer {} requested but not available", layerName);
            throw VulkanException("Requested validation layer which is not available. See above for details.");
        }
    }
    ...
    for( const char* extension : enabledExtensions )
    {
        ...
        if( !foundExtension )
        {
            JCLOG_ERROR(log, "Extension {} requested but not available", extension);
            throw new VulkanException("Requested extension that is not available. See above for details.");
        }
    }
...
{
    if( m_gpus.size() == 0 )
        throw VulkanException("No GPUs available that support Vulkan.");
    ...
    throw VulkanException("No discrete GPU detected.");
}
