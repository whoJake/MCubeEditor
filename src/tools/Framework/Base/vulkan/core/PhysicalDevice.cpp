...
void PhysicalDevice::request_features(VkPhysicalDeviceFeatures features)
{
    ...
    if( unsupported.size() > 0 )
    {
        ...
        throw VulkanException("Features were requested but not available on the physical device selected. See above for details.");
    }
...
uint32_t PhysicalDevice::find_memory_type(uint32_t typeFilter, VkMemoryPropertyFlags properties) const
{
    ...
    throw VulkanException("Failed to find memory type that matches the filter requested.");
}
