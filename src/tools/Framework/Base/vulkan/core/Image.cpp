...
inline static VkImageType get_image_type(VkExtent3D extent)
{
    ...
    case 3:
        ...
        break;
    default:
        throw new VulkanException("Failed to find requested image type for extent: {}, {}, {}", extent.width, extent.height, extent.depth);
    }
...
{
    if( mipLevels == 0 )
        throw new VulkanException("Image needs at least one mip level.");

    if( arrayLayers == 0 )
        throw new VulkanException("Image needs atleast one array layer.");
